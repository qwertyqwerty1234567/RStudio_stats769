...
y = automobile$price
(r.ridge = glmnet(x, y, alpha=0))
coef(r.ridge)
coef(r.ridge)[,10]

(r.lasso = glmnet(x, y, alpha=1))  # alpha = 1, lasso
names(r.lasso)
coef(r.lasso)
coef(r.lasso)[,10]
```
...
```{r}
(yhat = drop(predict(r.lasso, s=10, alpha=1, newx=x))) 
resid = automobile$price - yhat 
(mse = mean(resid^2))
```

## 12. Choose 5 different λ-values within a seemingly reasonable range and compute the MSEs of the corresponding 5 Lasso subset models. Write R code to find out how many variables are included in each Lasso subset model.
```{r}
